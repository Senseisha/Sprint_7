...
class TestAcceptOrder:
    ...
    def test_successful_order_accept(self, order_methods, colors, generate_couriers_data, courier_methods):
        body = helper.modify_create_order_body('color', colors)
        order_id = order_methods.create_order(body).json().get('track')

        login = generate_couriers_data['login']
        password = generate_couriers_data['password']
        courier_id = courier_methods.login_courier(login, password).json().get("id")

        response = order_methods.accept_order(order_id, courier_id)
        assert response.status_code == 200 and response.json()['ok'] is True
        with allure.step('Clear Up - delete courier after test'):
    ...
    def test_order_accept_without_courier_id(self, order_methods, courier_methods, generate_couriers_data, colors):
        body = helper.modify_create_order_body('color', colors)
        order_id = order_methods.create_order(body).json().get('track')
    ...
    @pytest.mark.parametrize('colors', [['BLACK']])
    def test_order_accept_with_wrong_courier_id(self, order_methods, courier_methods, generate_couriers_data, colors):
        body = helper.modify_create_order_body('color', colors)
        order_id = order_methods.create_order(body).json().get('track')
    ...
    def test_order_accept_without_order_id(self, order_methods, courier_methods, generate_couriers_data):
        order_id = ''
        login = generate_couriers_data['login']
        password = generate_couriers_data['password']
        courier_id = courier_methods.login_courier(login, password).json().get("id")
    ...
    def test_order_accept_with_wrong_order_id(self, order_methods, courier_methods, generate_couriers_data):
        order_id = '000'
        login = generate_couriers_data['login']
        password = generate_couriers_data['password']
        courier_id = courier_methods.login_courier(login, password).json().get("id")
